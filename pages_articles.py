BODY = """
<section class="page-hero">
  <div class="container">
    <span class="eyebrow">Articles &amp; Encouragement</span>
    <h1>Words for the <span class="script" style="font-size:1.05em">journey</span></h1>
    <p>Scripture-centered articles from Rev. Dr. Barbara A. F. Brehon on soul care, discipleship, sabbath rest, and spiritual growth — for pastors, ministry leaders, and everyone growing in grace.</p>
  </div>
</section>

<section>
  <div class="container">
    <div class="grid-3">
      <article class="card reveal">
        <span class="series-label">Soul Care for Pastors</span>
        <h3 style="font-size:1.25rem"><a href="/articles/signs-of-pastor-burnout" style="color:var(--plum-900)">Seven Signs of Pastor Burnout (and the Biblical Path Back)</a></h3>
        <p>Elijah burned out. Moses burned out. If you carry the ministry, learn the warning signs of ministry burnout — and God's gentle, biblical path back to renewal.</p>
        <div class="card-links"><a class="btn btn-sm btn-royal" href="/articles/signs-of-pastor-burnout">Read the Article</a></div>
      </article>
      <article class="card reveal">
        <span class="series-label">Coming Soon</span>
        <h3 style="font-size:1.25rem">Sabbath Rest for Leaders Who Can't Stop</h3>
        <p>Why the people who pour into everyone else find it hardest to receive — and how to build a rhythm of rest that actually holds.</p>
      </article>
      <article class="card reveal" style="justify-content:center;text-align:center">
        <h3 style="font-size:1.25rem">Never miss an article</h3>
        <p>New Scripture-centered articles are being poured out regularly. Start with the free 7-day devotional and stay connected.</p>
        <div class="card-links" style="justify-content:center"><a class="btn btn-sm btn-gold" href="/#devotional">Get the Free Devotional</a></div>
      </article>
    </div>
  </div>
</section>
"""
